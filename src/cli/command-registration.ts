...
function commandCwd(command: Command): string {
  const values: unknown = command.optsWithGlobals();
  if (typeof values === "object" && values !== null && "cwd" in values) {
...
export function registerMcpCommand(program: Command, config: ResolvedAcpxConfig): void {
  program
    .command("mcp")
    .description("Run the local multi-agent MCP facade over stdio")
    .action(async function (this: Command) {
      const { runMcpServer } = await import("../mcp-server.js");
      await runMcpServer({ cwd: commandCwd(this), config });
    });
}
...
export function registerDefaultCommands(program: Command, config: ResolvedAcpxConfig): void {
  ...
  registerMcpCommand(program, config);
}
