...
export type AcpRuntimeEnsureInput = {
  ...
  cwd?: string;
  /**
   * MCP servers scoped to this live runtime session. These values are applied
   * to session creation and reconnects but are never persisted in the session
   * record, so callers can safely rotate per-agent credentials.
   */
...
export type AcpRuntimeOptions = {
  ...
  probeAgent?: string;
  verbose?: boolean;
  /** Keep Claude user settings out of ACP-spawned sessions unless explicitly disabled. */
  isolateClaudeUserSettings?: boolean;
  onPermissionRequest?: (
