...
function assertFreshSessionAllowed(
  requireExistingSession: boolean | undefined,
  ...
  const reason = existing ? "is not compatible with the managed agent" : "was not found";
  throw new AcpRuntimeError(
    "ACP_SESSION_INIT_FAILED",
    `Persistent ACP session ${sessionKey} ${reason}; refusing to create a new session.`,
  );
}
...
export class AcpRuntimeManager {
  ...
  private rememberSessionMcpServers(recordId: string, mcpServers: McpServer[] | undefined): void {
    if (mcpServers !== undefined) {
      this.sessionMcpServers.set(recordId, structuredClone(mcpServers));
    }
  }

  private mcpServersFor(recordId: string): McpServer[] {
  ...
  async ensureSession(input: {
    ...
    const reused = await this.tryReuseEnsuredSession(input, existing, cwd, agentCommand);
    if (reused) {
      return reused;
    }
