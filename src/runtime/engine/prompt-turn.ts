...
type PromptTurnClient = {
  ...
  cancelActivePrompt?: (waitMs?: number) => Promise<unknown>;
  waitForSessionUpdatesIdle?: (options?: { idleMs?: number; timeoutMs?: number }) => Promise<void>;
};

async function cancelTimedOutPrompt(client: PromptTurnClient): Promise<void> {
  if (!client.cancelActivePrompt) {
    return;
  }
  await client.cancelActivePrompt().catch(() => undefined);
}
...
export async function runPromptTurn(params: {
  ...
  try {
    ...
    await cancelTimedOutPrompt(params.client);
    await params.client
      .waitForSessionUpdatesIdle?.({
        idleMs: SESSION_REPLY_IDLE_MS,
        timeoutMs: SESSION_REPLY_DRAIN_TIMEOUT_MS,
