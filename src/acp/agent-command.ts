...
export function resolveClaudeCodeExecutable(
  ...
  if (hasConfiguredClaudeCodeExecutable(platform, env)) {
    return undefined;
  }
  return platform === "win32"
    ? resolveWindowsExecutablePath("claude", env)
    : resolvePosixClaudeCodeExecutable(env);
}

function hasConfiguredClaudeCodeExecutable(
  platform: NodeJS.Platform,
  env: NodeJS.ProcessEnv,
): boolean {
  const executable =
    platform === "win32"
      ? readWindowsEnvValue(env, "CLAUDE_CODE_EXECUTABLE")
      : env.CLAUDE_CODE_EXECUTABLE;
  return !!executable?.trim();
}

function resolvePosixClaudeCodeExecutable(env: NodeJS.ProcessEnv): string | undefined {
