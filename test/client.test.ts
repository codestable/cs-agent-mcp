...
test("AcpClient can include Claude user settings for a managed runtime", async () => {
  const cwd = path.resolve("/tmp/acpx-client-claude-user-settings");
  const client = makeClient({
    agentCommand: "npx -y @agentclientprotocol/claude-agent-acp",
  ...
  asInternals(client).connection = {
    newSession: async (params: Record<string, unknown>) => {
      capturedParams = params;
      return { sessionId: "session-claude-user-settings" };
    },
  };

  await client.createSession("/tmp/acpx-client-claude-user-settings");
  assert.deepEqual(capturedParams, {
    cwd,
    mcpServers: [],
    _meta: {
