...
type MockAgentOptions = {
  ...
  pidFile?: string;
  newSessionIdFile?: string;
  loadSessionIdFile?: string;
...
function asObject(value: unknown, label: string): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error(`Injected MCP returned invalid ${label}`);
  }
...
async function delegateThroughInjectedMcp(
  session: SessionState,
  instruction: string,
  signal: AbortSignal,
): Promise<string> {
  const firstSpace = instruction.search(/\s/);
  if (firstSpace <= 0) {
    throw new Error("Usage: mcp-delegate <agent> <prompt>");
  }
  const agent = instruction.slice(0, firstSpace).trim();
  const prompt = instruction.slice(firstSpace + 1).trim();
  if (prompt.length === 0) {
    throw new Error("Usage: mcp-delegate <agent> <prompt>");
  }

  const server = session.mcpServers.find(
    (candidate): candidate is Extract<McpServer, { type: "http" }> =>
      "type" in candidate && candidate.type === "http" && candidate.name === "cs-agent-mcp",
  );
  if (!server) {
    throw new Error("cs-agent-mcp was not injected into the ACP session");
  }

  const headers = Object.fromEntries(server.headers.map((header) => [header.name, header.value]));
  const client = new Client({ name: "acpx-mock-agent", version: "1.0.0" });
  const closeOnAbort = () => {
    void client.close();
  };
  ...
  try {
    ...
    const createdContent = asObject(created.structuredContent, "create result");
    const createdAgent = asObject(createdContent.agent, "created agent");
    const agentId = requiredString(createdAgent.agentId, "created agent id");
    ...
    const sent = await client.callTool({
      ...
      arguments: {
        agentId,
        content: prompt,
        idempotencyKey: `mock-agent-${randomUUID()}`,
      },
    });
    const sentContent = asObject(sent.structuredContent, "send result");
    const receipt = asObject(sentContent.receipt, "send receipt");
    const turnId = requiredString(receipt.turnId, "turn id");
    ...
    const waitedContent = asObject(waited.structuredContent, "wait result");
    const result = asObject(waitedContent.result, "wait outcome");
    if (result.status !== "message") {
      throw new Error(`Injected MCP delegation did not return a message: ${String(result.status)}`);
    }
    const message = asObject(result.message, "result message");
    return requiredString(message.content, "result message content");
  } finally {
...
function parseMockAgentOptions(argv: string[]): MockAgentOptions {
  ...
  let newSessionIdFile: string | undefined;
  let loadSessionIdFile: string | undefined;
  let initializeMarker: string | undefined;
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === "--new-session-id-file") {
      newSessionIdFile = parseOptionValue(argv, index + 1, token);
      index += 1;
      continue;
    }

    if (token === "--load-session-id-file") {
      supportsLoadSession = true;
      loadSessionIdFile = parseOptionValue(argv, index + 1, token);
      index += 1;
      continue;
    }

    if (token === "--initialize-marker") {
      initializeMarker = parseOptionValue(argv, index + 1, token);
      index += 1;
      continue;
    }

    if (token === "--initialize-error") {
      initializeError = parseOptionValue(argv, index + 1, token);
      index += 1;
      continue;
    }
...
class MockAgent implements Agent {
  ...
  async initialize(): Promise<InitializeResponse> {
    if (this.options.initializeMarker) {
      writeFileSync(this.options.initializeMarker, "called", "utf8");
    }
    if (this.options.initializeError) {
      throw new Error(this.options.initializeError);
    }
  ...
  async newSession(params: NewSessionRequest): Promise<NewSessionResponse> {
    ...
    this.sessions.set(sessionId, createSessionState(false, [...params.mcpServers]));
    if (this.options.newSessionIdFile) {
      writeFileSync(this.options.newSessionIdFile, sessionId, "utf8");
    }
  ...
  async loadSession(params: LoadSessionRequest): Promise<LoadSessionResponse> {
    ...
    if (this.options.loadSessionIdFile) {
      writeFileSync(this.options.loadSessionIdFile, params.sessionId, "utf8");
    }
    ...
    const session = existing ?? createSessionState(false);
  ...
  private async handlePrompt(
    ...
    if (text.startsWith("mcp-delegate ")) {
      const instruction = text.slice("mcp-delegate ".length).trim();
      return await delegateThroughInjectedMcp(this.ensureSession(sessionId), instruction, signal);
    }
