...
test("AcpRuntimeManager refuses to create a fresh persistent session when resume is required", async () => {
  ...
  await assert.rejects(
    ...
    }),
    {
      ...
      message: /was not found; refusing to create a new session/,
    },
  );
  assert.equal(constructed, 0);
});

test("AcpRuntimeManager refuses an incompatible record when resume is required", async () => {
  const existing = makeSessionRecord({
    acpxRecordId: "moved-session",
    acpSessionId: "moved-backend-session",
    agentCommand: "codex --acp",
    cwd: "/other-workspace",
  });
  const store = new InMemorySessionStore([existing]);
  ...
  await assert.rejects(
    ...
    {
      code: "ACP_SESSION_INIT_FAILED",
      message: /is not compatible with the managed agent/,
    },
  );
  assert.equal(constructed, 0);
});
...
test("AcpRuntimeManager creates and resumes sessions through the client", async () => {
  ...
  let constructed = 0;
  const clientOptions: Array<ConstructorParameters<typeof AcpClient>[0]> = [];
  const manager = new AcpRuntimeManager(
    createRuntimeOptions({
      cwd: "/workspace",
      sessionStore: store,
      authCredentials: { codex: "configured-token" },
      authPolicy: "fail",
  ...
  assert.deepEqual(
    clientOptions.map((options) => ({
      authCredentials: options.authCredentials,
      authPolicy: options.authPolicy,
      isolateClaudeUserSettings: options.isolateClaudeUserSettings,
    })),
    [
      {
        authCredentials: { codex: "configured-token" },
...
test("AcpRuntimeManager restores persisted session env when reconnecting startTurn", async () => {
  ...
  assert.equal(
    (factoryCalls[0] as { isolateClaudeUserSettings?: boolean }).isolateClaudeUserSettings,
    false,
  );
});
...
test("AcpRuntimeManager forwards session-scoped MCP servers to a fresh client", async () => {
  const store = new InMemorySessionStore();
  const factoryCalls: Array<Record<string, unknown>> = [];
  const manager = new AcpRuntimeManager(
    createRuntimeOptions({ cwd: "/workspace", sessionStore: store }),
    {
      clientFactory: (options) => {
        ...
        return {
          ...
          start: async () => {},
          close: async () => {},
          createSession: async () => ({ sessionId: "mcp-sid", agentSessionId: "mcp-agent" }),
          loadSession: async () => ({ agentSessionId: "unused" }),
          hasReusableSession: () => true,
          supportsLoadSession: () => true,
          supportsResumeSession: () => false,
  ...
  const mcpServers = [
    {
      ...
      headers: [{ name: "Authorization", value: "Bearer child-token" }],
    },
  ];

  await manager.ensureSession({
    sessionKey: "session-scoped-mcp",
    agent: "codex",
    mode: "persistent",
    mcpServers,
  });

  assert.deepEqual(factoryCalls[0]?.mcpServers, mcpServers);
  assert.equal(JSON.stringify([...store.records.values()]).includes("child-token"), false);
});

test("AcpRuntimeManager keeps session-scoped MCP servers when reusing a persistent record", async () => {
  const existing = makeSessionRecord({
    acpxRecordId: "reused-mcp-session",
    acpSessionId: "reused-mcp-sid",
    agentCommand: "codex --acp",
    cwd: "/workspace",
  });
  const store = new InMemorySessionStore([existing]);
    ...
    createRuntimeOptions({ cwd: "/workspace", sessionStore: store }),
    {
      clientFactory: (options) => {
        ...
        return {
          ...
          supportsLoadSession: () => true,
          supportsResumeSession: () => false,
          loadSessionWithOptions: async () => ({ agentSessionId: "reused-agent" }),
          getAgentLifecycleSnapshot: () => ({ running: true }),
          prompt: async () => ({ stopReason: "end_turn" }),
          requestCancelActivePrompt: async () => false,
          hasActivePrompt: () => false,
  ...
  const mcpServers = [
    {
      ...
      name: "acpx-facade",
      url: "http://127.0.0.1:43123/mcp",
      headers: [{ name: "Authorization", value: "Bearer rotated-token" }],
    },
  ];

  const record = await manager.ensureSession({
    sessionKey: "reused-mcp-session",
    agent: "codex",
    mode: "persistent",
    mcpServers,
  });
  const result = await collectTurn(
    manager.startTurn({
      ...
      mode: "prompt",
      sessionMode: "persistent",
      requestId: "reused-mcp-turn",
    }),
  );

  assert.deepEqual(result.result, { status: "completed", stopReason: "end_turn" });
  assert.deepEqual(factoryCalls[0]?.mcpServers, mcpServers);
  assert.equal(JSON.stringify([...store.records.values()]).includes("rotated-token"), false);
});

test("AcpRuntimeManager rotates a warm persistent client when scoped MCP servers change", async () => {
  const store = new InMemorySessionStore();
  const factoryCalls: Array<Record<string, unknown>> = [];
  let closeCalls = 0;
  const manager = new AcpRuntimeManager(
    createRuntimeOptions({ cwd: "/workspace", sessionStore: store }),
    {
      clientFactory: (options) => {
        factoryCalls.push(options);
        const creationIndex = factoryCalls.length;
        return {
          initializeResult: { protocolVersion: 1, agentCapabilities: {} },
          start: async () => {},
          close: async () => {
          ...
          createSession: async () => ({ sessionId: "rotating-sid" }),
          loadSession: async () => ({ agentSessionId: "unused" }),
          hasReusableSession: () => true,
          supportsLoadSession: () => true,
          supportsResumeSession: () => false,
          loadSessionWithOptions: async () => ({ agentSessionId: `agent-${creationIndex}` }),
          getAgentLifecycleSnapshot: () => ({ running: true }),
          prompt: async () => ({ stopReason: "end_turn" }),
          requestCancelActivePrompt: async () => false,
          hasActivePrompt: () => false,
  ...
  const initial = await manager.ensureSession({
    sessionKey: "rotating-mcp-session",
    agent: "codex",
    mode: "persistent",
    mcpServers: mcpServers("first-token"),
  });
  await manager.ensureSession({
    sessionKey: "rotating-mcp-session",
    agent: "codex",
    mode: "persistent",
    mcpServers: mcpServers("second-token"),
  });
  await collectTurn(
    manager.startTurn({
      handle: createHandle(initial.acpxRecordId),
      text: "review",
      mode: "prompt",
      sessionMode: "persistent",
      requestId: "rotated-mcp-turn",
    }),
  );

  assert.equal(factoryCalls.length, 2);
  assert.equal(closeCalls, 1);
  assert.deepEqual(factoryCalls[1]?.mcpServers, mcpServers("second-token"));
});
