...
result 输出；mention 与媒体显示只读摘要，不输出媒体二进制。长内容按终端宽度换行，向上滚动会
暂停自动跟随并按新增行累计未读数，End 恢复实时跟随。会话内容直接来自 runtime 恢复已使用的
session record，不为 TUI 新建历史副本，也不启动 Agent。`top|ps` 要求 stdin/stdout 都是 TTY；
重定向或脚本场景应使用 `list --json` 或 `attach --json`，不会输出 ANSI 控制序列。
