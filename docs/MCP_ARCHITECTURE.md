...
- 消息与 Turn：`cs_agent_send`、`cs_agent_get_message`、`cs_agent_wait_message`、
  `cs_agent_wait_many`、`cs_agent_get_turn`、`cs_agent_wait_turn`
- 事件：`cs_agent_events`
...
兼容字段 `timeoutMs` 不向 ACP prompt 传播，也不限制 Agent 任务执行时间。

多 Agent fan-out 时，调用者先发送全部独立 Turn，再用 `cs_agent_wait_many` fan-in。any 返回当前
snapshot 中全部 ready 项；all 等待 pending 为空，但权限请求会提前返回。等待结果保持输入顺序，
重复 Turn ID 按首次出现折叠；任一 Turn 不存在或不在 actor 子树内时整批失败。权限或 timeout
中断 all 后，调用者按 Turn ID 累计 ready，并只续等 `pendingTurnIds`。
...
`listAgents()`、`attachAgent()` 和 `readConversation()` 读接口；`readConversation()` 用
`mcp-<rootExecutionId>-<agentId>` 将 persistent Agent 映射到固定 record，并枚举同前缀的
...
diagnostics DTO，剥离终端控制序列并按显示宽度换行；paused/live 以实际渲染行维护视口，未读数按
新增 conversation item 计算。loading、等待首条消息、空会话和历史会话不可用是四个独立状态；
不可用状态同时显示已持久化的 runtime 错误。
