...
4. **attach 保持 Agent 级、只读和长连接语义。** 它输出初始状态、有限历史，再从该 snapshot 的
   ...
   停止或 lock generation 改变时先做一次最终 cursor drain，再以非零退出。`Ctrl-C` 只停止观察，
   输出 `interrupted` terminal 并返回 0。
...
- Seam：Facade persistence 导出“验证后只读 snapshot file”窄入口，对 CLI 会消费的 Agent/Turn/
  Permission/Event nested fields 做 L3 required-field/type 校验，允许未知兼容字段但不透传；ENOENT 与
  invalid 分开返回。process-lock 导出不清理文件的 probe，内部返回 PID/token/createdAt 与
  running/stopped/unknown；token 只供 generation 比较。CLI 与测试都穿过 AgentDiagnostics，不直接
  解析 JSON。
...
- attach 内部记住初始 lock token 作为 generation，但不渲染、不记录 token。lock 消失、PID 不再
  存活或 token 改变时，先最终读取当前 snapshot 并排空剩余目标事件，再输出 terminal；不跨 Facade
  generation 无缝跟随。
...
- **终止语义**：Agent destroyed 后输出该事件与 terminal 并返回 0；Facade 停止、unknown 或被新
  generation 替换时最终 drain 后输出 terminal 并返回 1；Agent 从 running 变 idle 不自动退出。
...
- npm binary `cs-agent-mcp` 的 Commander 命令树：新增 `agents list|status|attach`，保留默认 action。
...
- 文件级 — `src/mcp/facade/store.ts`：166 行，读写/校验职责聚合；增加只读验证入口属于原职责。
...
新逻辑直接落独立 diagnostics 模块；现有 store/lock 只扩展窄只读接口。Facade 大文件拆分属于
行为等价重构，另开 `cs-refactor` 才处理，不作为本 feature 前置。
...
8. Facade shutdown 或 lock generation 更替 → 先最终 drain release 前的末尾事件，再输出 terminal
   并返回 1；`Ctrl-C` 输出 interrupted terminal 并返回 0，且不跨新实例继续。
